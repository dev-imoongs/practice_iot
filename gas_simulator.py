...
class GasSensor:
    def __init__(self, gas_type: str, min_val: float, max_val: float):
...
class GasDetector:
    ...
    def get_sensor_data(self) -> List[Tuple[str, float, bool]]:
        result = []
        for sensor in self.sensors:
            value = sensor.generate_value()
            # 단순한 이상 감지 로직 (100~200 사이면 정상)
            is_normal = 100 < value < 200
            result.append((sensor.gas_type, value, is_normal))
        return result
